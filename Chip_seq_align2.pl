...
while (<IN>) {
	...
	my $cmd;
	if($type eq 'pair'){
		$cmd="perl ~/scripts/pairbam2bed_bwa.pl $sample_name.bam pair > $sample_name.bam.pair.bed";
	}
	elsif($type eq 'single'){
		$cmd="perl ~/scripts/pairbam2bed_bwa.pl $sample_name.bam real_single > $sample_name.single.bed";
	}
	print O $cmd,"\n";
}
close IN;
...
while (<IN>) {
	...
	my $cmd="~/soft/IGVTools/igvtools count -z 10 -w 100 -e $tmp_e $sample_name.bam $sample_name.bam.tdf hg19";
	#print "$cmd\n";
	print O $cmd,"\n";
}
close IN;
...
while (<IN>) {
	...
	my $cmd;
	if($type eq 'pair'){
	if(-e "$infor[0]" and -e "$infor[1]"){
		$cmd="perl /home/jhmi/xinli/scripts/Calculate_enrich_ratio2_for_bam_file.pl $infor[0] $infor[1] $name1 $name2 ~/amber3/Oliver/Chip-Seq/hg19.len $window_size $step_size hg19";
	}
...
while (<IN>) {
	...
	my $cmd="perl ~/scripts/sort_file_pid.pl $infor[0].$type.bed ~/amber3/Akiko/Chip-Seq/human_hiseq031/hg19.len $$.$index ";
	$cmd.="&& perl ~/scripts/sort_file_pid.pl $infor[1].$type.bed ~/amber3/Akiko/Chip-Seq/human_hiseq031/hg19.len $$.$index ";
