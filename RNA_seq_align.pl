...
my ($sample_list,$ref,$help,$step,$ref_dir,$gene_model,$gtf,$gtf_exon);
...
if (!defined $ref) {
$ref_dir="/amber3/feinbergLab/personal/xinli/no_back_up/data/genome/hg19_OSA/Human.B37";
$ref="Human.B37";
$gene_model="RefGene";
}
...
while (<IN>) {
	...
	while (my $file=readdir(DIR)) {
		if ($file=~/.*_R1_.*fastq.gz$/ or $file=~/\.fastq$/ or $file=~/\.read1.fq.gz$/ or $file=~/\.read1.fq/) {
			my $pair=$file;
			$pair=~s/R1/R2/;
			$pair=~s/read1/read2/;
			my $zip="True";
			if($file=~/\.fastq$/ or $file=~/\.fq/){$zip="False"}
			my $cmd="perl /home/jhmi/xinli/scripts/Run_OSA.pl $_/$file $_/$pair $ref_dir $ref $gene_model $file $zip";
			my $cmd2=`$cmd`;
			print O $cmd2,"\n";
		}
...
while (<IN>) {
	chomp;
	my $sample_name=(split /\//,$_)[-1];
	my $cmd= "/home/jhmi/xinli/soft/IGVTools/igvtools count -z 5 -w 25 -e 100 $sample_name.sort.bam $sample_name.sort.bam.tdf hg19";
	print O $cmd,"\n";
}
close IN;
close O;
